...
from aa_api_wrapper.main import app
...
@pytest.fixture
def headers():
    return {"Authorization": f"Bearer {os.getenv('AA_TOKEN')}"}
...
def test_completions(client, mock_aleph_alpha_client, headers):
    mock_response = MagicMock()
    mock_response.json.return_value = {
        "choices": [{"text": "Berlin is the capital of Germany."}]
    }
    mock_aleph_alpha_client.request.return_value = mock_response
...
def test_embeddings(client, mock_aleph_alpha_client, headers):
    ...
    response = client.post(
        "/v1/embeddings",
        json={"model": "luminous-base", "input": "Apple"},
        headers=headers,
    )
    ...
    assert response.json()["data"][0]["embeddings"]["layer_40"]["mean"] == [
        0.1,
        0.2,
        0.3,
    ]
